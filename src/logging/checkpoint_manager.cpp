...
namespace logging {
  ...
  void CheckpointManager::Running() {
    ...
    while (1) {
      ...
      if (count == checkpoint_interval_ || is_init == true) {
        ...
        cid_t begin_cid = epoch_manager.EnterEpoch(0,TimestampType::READ);
 ...
 //   concurrency::Transaction* current_txn = new concurrency::Transaction(thread_id, IsolationLevelType::SERIALIZABLE, current_cid);
