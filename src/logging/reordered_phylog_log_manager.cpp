...
void ReorderedPhyLogLogManager::RegisterWorker() {
  ...
  //Only 1 logger thread for now
  loggers_[0]->RegisterWorker(tl_worker_ctx);
}

// deregister worker threads.
void ReorderedPhyLogLogManager::DeregisterWorker() {
  PL_ASSERT(tl_worker_ctx != nullptr);
  loggers_[0]->DeregisterWorker(tl_worker_ctx);
}
...
void ReorderedPhyLogLogManager::WriteRecordToBuffer(LogRecord &record) {
  ...
  auto buffer_ptr = ctx->buffer_pool.GetBuffer(0);
  ...
  if (is_success == false) {
    ...
    //buffer_ptr = ctx->buffer_pool.GetBuffer(0);RegisterNewBufferToEpoch(std::move((ctx->buffer_pool.GetBuffer(ctx->current_commit_eid))));
    // Write it again
...
void ReorderedPhyLogLogManager::StartPersistTxn(concurrency::Transaction* txn) {
  // Log down the begin of a transaction
  LogRecord record = LogRecordFactory::CreateTxnRecord(LogRecordType::TRANSACTION_BEGIN, txn->GetTransactionId());
  WriteRecordToBuffer(record);
}

void ReorderedPhyLogLogManager::EndPersistTxn(concurrency::Transaction* txn) {
  PL_ASSERT(tl_worker_ctx);
  //TODO: Needs to handle abort as well
  LogRecord record = LogRecordFactory::CreateTxnRecord(LogRecordType::TRANSACTION_COMMIT, txn->GetTransactionId());
  WriteRecordToBuffer(record);
}
...
void ReorderedPhyLogLogManager::StartTxn(concurrency::Transaction *txn) {
 ...
 // size_t cur_eid = concurrency::EpochManagerFactory::GetInstance().GetCurrentEpochId();
  ...
  //size_t epoch_idx = cur_eid % 5; //concurrency::EpochManager::GetEpochQueueCapacity();
  ...
  DurabilityFactory::StartTxnTimer(txn_cid, tl_worker_ctx);
...
//    std::this_thread::sleep_for(std::chrono::microseconds//      (concurrency::EpochManagerFactory::GetInstance().GetEpochLengthInMicroSecQuarter())
//                        (10000)
//                                );

//    size_t curr_persist_epoch_id = 9999;//MAX_EPOCH_ID;
...
//    PL_ASSERT(curr_persist_epoch_id < 99999);//MAX_EPOCH_ID);
